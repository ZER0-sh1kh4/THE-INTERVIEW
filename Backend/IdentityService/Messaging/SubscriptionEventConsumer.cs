...
/// <summary>
/// Consumes subscription saga events and updates the user's premium state.
/// </summary>
public class SubscriptionEventConsumer : BackgroundService
{
    ...
    public SubscriptionEventConsumer(IOptions<RabbitMqOptions> rabbitMqOptions, IServiceScopeFactory scopeFactory, ILogger<SubscriptionEventConsumer> logger)
    {
    ...
    /// <summary>
    /// Opens RabbitMQ resources and starts listening for subscription lifecycle events.
    /// </summary>
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await DeclareQueueWithDeadLetterAsync(QueueNames.SubscriptionLifecycle, stoppingToken);

        var consumer = new AsyncEventingBasicConsumer(_channel);
        consumer.ReceivedAsync += async (_, eventArgs) => await HandleDeliveryAsync(eventArgs, stoppingToken);

        await _channel.BasicConsumeAsync(QueueNames.SubscriptionLifecycle, autoAck: false, consumer: consumer, cancellationToken: stoppingToken);
    }

    /// <summary>
    /// Processes one saga event and retries or dead-letters it when needed.
    /// </summary>
    private async Task HandleDeliveryAsync(BasicDeliverEventArgs eventArgs, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (message.Action is not ("Activate" or "Cancel"))
            {
                await _channel!.BasicAckAsync(eventArgs.DeliveryTag, false, cancellationToken);
                return;
            }
        ...
        {
            _logger.LogError(ex, "Failed to process subscription lifecycle event.");
            await RetryOrDeadLetterAsync(QueueNames.SubscriptionLifecycle, eventArgs, cancellationToken);
        }
    }

    /// <summary>
    /// Applies the premium state change and publishes the saga result back to subscription service.
    /// </summary>
    private async Task HandleMessageAsync(SubscriptionLifecycleEvent message, CancellationToken cancellationToken)
    {
        ...
        var user = await dbContext.Users.FirstOrDefaultAsync(u => u.Id == message.UserId, cancellationToken);
        if (user == null)
        {
            await publisher.PublishAsync(
                QueueNames.SubscriptionResults,
                new SubscriptionLifecycleEvent
                {
        ...
        await publisher.PublishAsync(
            ...
            new SubscriptionLifecycleEvent
            {
                ...
                Action = message.Action,
                Status = "Completed",
                Message = $"Premium state updated to {user.IsPremium}."
            },
            cancellationToken);
    }
